...
class OperatorButton(pygame.sprite.Sprite):
        ...
        if self.rect.collidepoint(pygame.mouse.get_pos()):
            ...
            if firstNum and not operation:  # First number already entered, operator just entered
                ...
                if operator == '=/CLEAR':  # Clear
                    ...
                    # if outputTextArea.text[0] == '-':  # Check for negative number
                    #    firstVal = -firstVal
                    operation = True
                ...
                if operator == '=/CLEAR':  # Clear
                    ...
                    # Invalid operator used, send error code to calculate function for error handling
                    self.calculate(-1, operator, secondVal)
    ...
    def calculate(self, first, op, second = None):
        ...
        if first == -1:  # Error checking for invalid input
            outputTextArea.update()
        ...
        if type(answer) is str:
            ...
            outputTextArea.update("{:.2f}".format(answer))
    ...
    def clicked(self, text, outputTextArea):
        ...
        if self.rect.collidepoint(pygame.mouse.get_pos()):
            ...
            if not firstNum:  # First number being entered
                ...
                # Set global first value and set boolean checker to true
                if type(text) is not str:
